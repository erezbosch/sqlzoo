...
def prolific_actors
  ...
  execute(<<-SQL)
    ...
    JOIN
      castings ON movies.id = castings.movie_id
    JOIN
      actors ON castings.actor_id = actors.id AND castings.ord = 1
    GROUP BY
      actors.name
    HAVING
...
def films_by_cast_size
  ...
  FROM
    movies
  JOIN
    castings ON movies.id = castings.movie_id AND movies.yr = 1978
  JOIN
    actors ON castings.actor_id = actors.id
  GROUP BY
    movies.title
...
def colleagues_of_garfunkel
  ...
  FROM
    castings
  JOIN
    actors ON castings.actor_id = actors.id AND castings.movie_id IN (
      SELECT
        movie_id
      FROM
        castings
