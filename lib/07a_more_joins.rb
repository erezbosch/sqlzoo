...
def heart_tracks
  ...
  execute(<<-SQL)
    ...
    FROM
      albums
    JOIN
      tracks ON albums.asin = tracks.album AND tracks.song LIKE '%Heart%'
    GROUP BY
      albums.title
    ORDER BY
...
def title_tracks
  ...
  execute(<<-SQL)
    ...
    FROM
      tracks
    JOIN
      albums ON tracks.album = albums.asin AND tracks.song = albums.title
  SQL
end
...
def best_value
  ...
  execute(<<-SQL)
    SELECT
      albums.title, price, COUNT(tracks.song)
    FROM
      albums
    JOIN
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, SUM(price)/SUM(num_songs)
    FROM
      styles
    JOIN
      (
      SELECT
        albums.asin asin, albums.price price, COUNT(tracks.song) num_songs
      FROM
        albums
      JOIN
        tracks ON tracks.album = albums.asin
