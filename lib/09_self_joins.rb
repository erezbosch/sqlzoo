...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      a.num, a.company, stops.name, second_bus_num, second_bus_company
    FROM
      routes a
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num) AND a.stop_id = 53
    JOIN (
      SELECT
        d.company second_bus_company, d.num second_bus_num, c.stop_id midpt_id
      FROM
        routes c
      JOIN
      ...
      WHERE
        d.stop_id = 213
      ) second_route ON b.stop_id = midpt_id
    JOIN
